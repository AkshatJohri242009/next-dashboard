  ...
  return (
    <div className={cn("glass rounded-2xl p-3 sm:p-5 relative overflow-hidden", allDone && "ring-1 ring-brand-400/20")}>
      ...
      <Reorder.Group axis="y" values={goals} onReorder={reorderGoals} className="space-y-1.5">
        <AnimatePresence mode="popLayout">
          {goals.map((goal, idx) => (
              ...
              as="div"
              className={cn(
                "flex items-center gap-1.5 sm:gap-3 px-1.5 sm:px-3 py-2 sm:py-2.5 rounded-lg sm:rounded-xl border border-white/[0.06] bg-white/[0.02] transition-colors group",
      ...
      <div className="flex gap-2 mt-4 pt-4 border-t border-white/[0.06]">
        ...
        <button
          onClick={handleAdd}
          className="h-9 px-2.5 sm:px-3 rounded-lg bg-brand-500 text-black text-xs font-bold hover:bg-brand-400 transition-colors flex items-center gap-1 shadow-lg shadow-brand-500/20 shrink-0"
        >
