        ...
        <div className="flex gap-2">
          {goals.some(g => !g.done) && (
            <button
              ...
              className="flex items-center gap-1.5 h-8 px-3 rounded-xl text-[11px] font-bold font-mono text-white/40 bg-white/[0.04] border border-white/[0.06] hover:text-white/70 hover:bg-white/[0.08] transition-colors"
            >
        ...
        <AnimatePresence mode="popLayout">
          {goals.map((goal, idx) => (
            <Reorder.Item
              ...
              as="div"
              className={cn(
                "flex items-center gap-3 px-3 py-2.5 rounded-xl border border-white/[0.06] bg-white/[0.02] transition-colors",
                goal.done && "opacity-40",
                glowClass(goal.pushedCount || 0),
              )}
              style={{ position: "relative" }}
            >
              <GripVertical className="w-3.5 h-3.5 text-white/20 cursor-grab active:cursor-grabbing shrink-0 opacity-0 group-hover:opacity-100" />

              <input
                type="checkbox"
                checked={goal.done}
                onChange={() => toggleGoal(idx)}
                className="w-4.5 h-4.5 rounded-lg appearance-none border-1.5 border-white/20 bg-black/20 checked:bg-brand-400 checked:border-brand-400 cursor-pointer shrink-0
              ...
              <button
                ...
                className="h-7 w-7 rounded-lg flex items-center justify-center text-white/30 hover:text-red-400 hover:bg-red-400/10 transition-colors"
              >
