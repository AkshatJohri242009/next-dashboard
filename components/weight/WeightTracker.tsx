    ...
    <div className="space-y-4">
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        <div>
          <div className="flex items-baseline gap-1">
            ...
            <span className="text-lg text-text-tertiary">kg</span>
            {change !== null && (
              <span className={`flex items-center gap-1 text-sm font-bold ml-2 ${Number(change) > 0 ? "text-red-400" : Number(change) < 0 ? "text-brand-400" : "text-text-tertiary"}`}>
          ...
          <>
            <div>
              ...
              <div className="mt-1">
                ...
                <span className="text-xs text-text-tertiary ml-1">kg</span>
              </div>
            </div>
            <div>
              <span className="section-label">7-day Trend</span>
              <div className="mt-1">
                <span className={`text-[28px] font-bold tabular-nums ${weekDelta > 0 ? "text-red-400" : weekDelta < 0 ? "text-brand-400" : "text-text-secondary"}`}>
      ...
      <div className="grid grid-cols-2 sm:flex sm:flex-wrap gap-2 items-end">
        <label className="flex flex-col gap-1">
          <span className="text-xs font-mono font-extrabold tracking-wider text-text-tertiary uppercase">Weight (kg)</span>
        ...
        <label className="flex flex-col gap-1">
          <span className="text-xs font-mono font-extrabold tracking-wider text-text-tertiary uppercase">Note</span>
          <input value={note} onChange={e => setNote(e.target.value)}
            onKeyDown={e => { if (e.key === "Enter") add() }}
            placeholder="Optional..."
            className="h-10 w-full sm:w-40 px-3 rounded-xl bg-white/[0.04] border border-white/[0.06] text-sm text-white outline-none placeholder:text-text-tertiary focus:border-white/20 transition-colors" />
        </label>
        <button
          onClick={add}
      ...
      {entries.length > 1 && (
        <>
          <div className="glass-strong rounded-xl p-4">
            <div className="text-[11px] font-mono font-bold text-text-tertiary mb-3 uppercase">Weight Trend</div>
          ...
          <div className="glass-strong rounded-xl p-4">
            <div className="text-[11px] font-mono font-bold text-text-tertiary mb-3 uppercase">Changes</div>
      ...
      <div className="space-y-1">
        {entries.slice().reverse().map((e, ri) => {
          ...
          return (
            ...
            >
              <div>
                <span className="text-sm font-bold text-text-primary">{e.weight} kg</span>
                <span className="text-xs text-text-tertiary ml-3 font-mono">{e.date}</span>
                {e.note && <span className="text-xs text-text-tertiary ml-2">· {e.note}</span>}
              </div>
              <button onClick={() => remove(idx)} className="h-9 w-9 sm:h-7 sm:w-7 rounded-xl sm:rounded-lg flex items-center justify-center text-text-tertiary hover:text-red-400 hover:bg-red-400/10 transition-colors">
        ...
        {entries.length === 0 && (
          <div className="py-8 text-center text-sm text-text-tertiary italic">No weight entries yet. Log your first one above.</div>
        )}
      </div>
    </div>
  )
