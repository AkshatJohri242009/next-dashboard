    ...
    <div className="space-y-4">
      <div className="p-4 rounded-xl bg-gradient-to-br from-brand-500/5 to-accent-500/5 border border-white/[0.06]">
        <div className="flex items-center gap-2 mb-2">
          ...
          <span className="text-xs font-medium text-white/40">Pattern Insight</span>
        </div>
        <p className="text-sm text-white/70">{insight}</p>
      </div>

      {correlations.length === 0 && (
        <p className="text-center text-sm text-white/30 py-8">Track your life for 7+ days to discover patterns. The more data, the smarter the insights.</p>
      )}
              ...
              return (
                ...
                >
                  <div className="flex items-center justify-between mb-2">
                    <div className="flex items-center gap-2">
                      <div className="flex items-center gap-1">
                        <IconA className="w-3.5 h-3.5 text-white/40" />
                        <span className="text-xs text-white/30">{corr.sourceA}</span>
                      </div>
                      <DirectionIcon className={cn("w-3.5 h-3.5", corr.direction === "positive" ? "text-success" : "text-danger")} />
                      <div className="flex items-center gap-1">
                        <IconB className="w-3.5 h-3.5 text-white/40" />
                        <span className="text-xs text-white/30">{corr.sourceB}</span>
                      </div>
                    </div>
                  ...
                  <div className="flex items-center gap-3 mt-1.5">
                    <span className="text-[11px] text-white/30">{corr.sourceA}: {typeof corr.valueA === 'number' ? corr.valueA.toFixed(1) : corr.valueA}</span>
                    <Minus className="w-3 h-3 text-white/20" />
                    <span className="text-[11px] text-white/30">{corr.sourceB}: {typeof corr.valueB === 'number' ? corr.valueB.toFixed(1) : corr.valueB}</span>
                  </div>
                </motion.div>
              )
            })}
