...
export function gatherContext(page: string): JarvisContext {
  ...
  try {
    ...
    goals = { total: g.length, done: g.filter((x: any) => x.done).length }
  } catch {}
  ...
  try {
    ...
    if (g.logs) {
      const thisWeek = g.logs.filter((l: any) => {
        const d = new Date(l.at).toISOString().slice(0, 10)
        const weekAgo = new Date(Date.now() - 7 * 86400000).toISOString().slice(0, 10)
        return d >= weekAgo
      })
      gym = {
        sessionsThisWeek: new Set(thisWeek.map((l: any) => new Date(l.at).toISOString().slice(0, 10))).size,
  ...
  try {
    const t = JSON.parse(localStorage.getItem("study_tasks_v1") || "[]")
    study.tasksTotal = t.length
    study.tasksDone = t.filter((x: any) => x.done).length
    const scores = JSON.parse(localStorage.getItem("study_scores_v1") || "[]")
    if (scores.length > 0) {
      study.avgScore = Math.round(scores.reduce((a: any, s: any) => a + (s.score || 0) / (s.total || 1), 0) / scores.length * 100)
    }
  } catch {}
  ...
  try {
    const p = JSON.parse(localStorage.getItem("tracked_projects") || "[]")
    projects = {
      activeProject: p.find((x: any) => x.startTime)?.name || null,
  ...
  try {
    const h = JSON.parse(localStorage.getItem("lifeos_habits") || "[]")
    habits = {
      total: h.length,
      doneToday: h.filter((x: any) => x.logs?.includes(today)).length,
      longestStreak: Math.max(0, ...h.map((x: any) => x.streak || 0)),
    }
  } catch {}
  ...
  try {
    const d = JSON.parse(localStorage.getItem("lifeos_decisions") || "[]")
    decisions = {
      totalDecisions: d.length,
      positiveOutcomes: d.filter((x: any) => x.outcome === "positive").length,
      recentDecision: d.length > 0 ? d[d.length - 1].title || d[d.length - 1].question : null,
    }
  } catch {}
  ...
  try {
    const m = JSON.parse(localStorage.getItem("lifeos_memory_engine") || "[]")
    const catCount: Record<string, number> = {}
    m.forEach((x: any) => { if (x.category) catCount[x.category] = (catCount[x.category] || 0) + 1 })
    const topCat = Object.entries(catCount).sort((a, b) => b[1] - a[1]).slice(0, 3).map(e => e[0])
    memories = {
      total: m.length,
      recentMemories: m.slice(-3).map((x: any) => x.text || x.title || x.content || ""),
  ...
  try {
    const ms = JSON.parse(localStorage.getItem("lifeos_missions") || "[]")
    missions = {
      total: ms.length,
      completed: ms.filter((x: any) => x.status === "completed").length,
      activeCount: ms.filter((x: any) => x.status === "active").length,
    }
  } catch {}
  ...
  try {
    const j = JSON.parse(localStorage.getItem("lifeos_journal") || "[]")
    journal = {
      totalEntries: j.length,
      recentMood: j.length > 0 ? j[j.length - 1].mood || j[j.length - 1].emoji : null,
    }
  } catch {}
  ...
  try {
    const scores = JSON.parse(localStorage.getItem("study_scores_v1") || "[]")
    const weak = scores.filter((s: any) => (s.score || 0) < ((s.total || 1) * 0.6))
    weakTopics = [...new Set(weak.map((s: any) => s.topic || s.subject || "").filter((x: unknown) => x)) as Set<string>].slice(0, 3)
  } catch {}
  ...
  try {
    ...
    const doneToday = tasks.filter((t: any) => t.done && t.date === today)
